...
> 적용하는 두 가지 방식: 항시 발동 bash 설치형과 명시적 발동 Claude Code
> 플러그인.
>
...
| | 항시 발동 모드 | 명시적 발동 모드 |
|---|---|---|
| 메커니즘 | bash 설치 스크립트가 대상 레포 파일에 콘텐츠 주입 | Claude Code 플러그인을 슬래시 커맨드로 호출 |
| 워크플로우 적용 시점 | 매 세션 자동 | `/bds-workflow` 명시적 호출 시에만 |
| 배포 채널 | `curl | bash` | Claude Code 플러그인 marketplace |
| 에이전트 호환성 | `AGENTS.md`를 읽는 모든 에이전트 (Claude Code, Cursor, Codex, …) | Claude Code 전용 |
| 이런 사람에게 | 매번 의식하지 않아도 워크플로우가 적용되길 원하거나 여러 AI 에이전트 도구를 함께 씀 | Claude Code만 쓰며 작업별로 명시적으로 켜고 싶음 |
...
## 명시적 발동 모드 (Claude Code 플러그인)

beads 워크플로우 규약을 세 개의 skill로 노출하는 Claude Code 플러그인입니다.
사용자가 명시적으로 호출했을 때만 로드됩니다. 레포의 작업 파일은 수정하지
않으며, 모든 콘텐츠는 플러그인 설치 위치에 존재합니다.
...
세 skill 모두 슬래시 커맨드 호출 시에만 동작하며, 세션 내용에 기반해 자동
발동되지 않습니다.
...
Claude Code 세션에서 beads 관련 작업을 시작할 때:

```
/bds-workflow
```

10단계 워크플로우 규칙을 로드합니다. 세션이 끝날 때까지 컨텍스트에 남아
있습니다.
...
마커 존재)에 플러그인을 추가하려면, 워크플로우 규칙 중복 로드를 피하기 위해
먼저 항시 발동 모드를 언인스톨하세요:
