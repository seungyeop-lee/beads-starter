...
beads 이슈 등록, `bd` 상태 전환은 설치 대상 레포에 주입되는 운영 규칙이지
...
└── docs/beads-starter/
    ├── bd-setup.md.part       # → 대상 `docs/beads-starter/bd-setup.md` 마커 구간
    └── beads-commands.md.part # → 대상 `docs/beads-starter/beads-commands.md` 마커 구간
```
...
- `docs/*.md.part`는 각각 H1으로 시작합니다. starter가 새로 만드는 파일이기
  때문입니다.
- `docs/beads-starter/bd-setup.md.part`의 `## Expected warnings` 섹션은 현재 프리셋 플래그
...
3. `update` — 미설치 상태에서는 에러, 설치 상태에서는 기존 prefix를
   `docs/beads-starter/bd-setup.md`의 `bd init --prefix X` 라인에서 역추출해 멱등 재주입.
