...
업데이트** — README(루트·`always-on/`·`on-demand/`의 영/한 페어), AGENTS.md,
skill description, 페이로드 내 참조.

## README 관리

`README.md`(영어, 기본)와 `README.ko.md`(한국어) 페어가 세 곳에 존재합니다:

- 루트 — 모드 선택 진입점.
- `always-on/` — 항시 발동 모드의 절차.
- `on-demand/` — 명시적 발동 모드의 절차.

각 페어는 **같은 커밋 안에서 항상 내용이 일치**해야 합니다.

- 한쪽의 섹션을 추가·삭제·수정하면 같은 디렉터리의 다른 언어 파일도 **같은
  커밋에서** 동일하게 반영합니다. 번역 지연을 이유로 한쪽만 선행 머지하지
  마십시오 — 다른 언어 사용자가 잘못된 정보를 보게 됩니다.
- 각 파일 상단의 언어 스위처 줄(`*English · [한국어](README.ko.md)*` /
  `*[English](README.md) · 한국어*`)은 같은 디렉터리 내 페어의 유일한 연결
  지점입니다. 제거하거나 경로를 바꾸면 상호 이동이 깨집니다.
- 파일명(`README.md`, `README.ko.md`)은 GitHub가 `README.md`를
  자동 렌더링한다는 사실에 의존한 계약입니다. 영어가 기본이어야 합니다.
- 새 언어를 추가한다면 `README.<lang>.md` 규약을 따르고, **세 디렉터리
  모두**의 기존 파일 스위처 줄에 새 링크를 추가하십시오.

### 루트 vs 모드 README 역할 분리

- **루트 README** — 헤더, 모드 비교표, 모드 상호배타 안내, 각 모드 README
  진입 링크, 두 모드 공통 사실(Operating mode), 라이선스만 둡니다. 모드별
  절차(설치/업데이트/언인스톨 등)를 루트에 중복 보관하지 마십시오.
- **모드 README** — 자기 모드의 절차에만 집중합니다. 다른 모드를 언급하거나
  교차 참조하지 마십시오 — "두 모드가 존재하고 상호 배타다"는 사실은 루트가
  단일 정보원입니다. 한 모드에서 다른 모드로 전환하는 안내가 필요해지면
  루트에 추가하십시오.
